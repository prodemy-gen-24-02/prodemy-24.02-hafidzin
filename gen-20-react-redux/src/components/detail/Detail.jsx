...
export default function Detail({ products }) {
  ...
  const onSubmit = (data) => {
    const sameId = dataCheckout.find((d) => d.id === data.id);
    const inputData = () => {
      if (sameId !== undefined) {
        const qty = { qty: parseInt(sameId.qty) + 1 };
        const updateData = dataCheckout.map((p) => {
          if (p.id == id) {
            return { ...p, ...qty };
          } else {
            return p;
    ...
    dispatch(checkoutBooking(inputData()));
  };
    ...
    <>
      <div className="grid grid-cols-2 max-lg:grid-cols-1 gap-6 lg:mx-32 mx-4 justify-between py-12">
        <div className="max-lg:order-2">
          ...
          <h2 className="mt-4 mb-3 text-3xl font-bold font-sans bg-transparent">
            &euro;{data?.price}
            <input value={data?.price} type="hidden" {...register("price")} />
          </h2>
          ...
          >
            <div className="flex flex-row items-center gap-2 mb-2 z-10">
              ...
              <input type="hidden" value={data.src[0]} {...register("img")} />
              <input
